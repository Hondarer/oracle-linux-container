...
このプロジェクトでは、次の方針でコンテナイメージを管理します。
...
Git タグ `v20251116.0.0` を作成した場合、次のイメージタグが自動生成されます。
...
`main` ブランチへの push では、次のイメージタグが生成されます。
...
1. GitHub の Releases ページで新しいリリースを作成します。
2. タグ名を入力します (例: `v20251116.0.0`)。
3. リリースノートを記述します。
...
1. Git タグを作成したか (ブランチへの push では latest は更新されません)
2. より新しいタグが既に存在していないか
