...
プロジェクトをチェックアウトし、ビルドとテストを実行する基本的なパターンです。

詳細は [基本的な使い方](./basics.md) を参照してください。

### パターン 2: 複数言語のプロジェクト

C++、Node.js、Python など複数の言語が混在するプロジェクトでの利用パターンです。

各言語のサンプルドキュメントを組み合わせて設定します。

### パターン 3: ドキュメント自動生成

ソースコードから自動的にドキュメントを生成し、GitHub Pages にデプロイするパターンです。

詳細は [ドキュメント生成](./documentation-example.md) を参照してください。

### パターン 4: マトリクステスト

複数のバージョンやパラメータで並列テストを実行するパターンです。

詳細は [高度な設定](./advanced-configuration.md#ジョブマトリクスによる複数バージョンテスト) を参照してください。
