...
複数のバージョンでテストする場合（このコンテナには Node.js 24(OL8)/22(OL10) がインストール済み）：
...
**注意**: このコンテナイメージを使用する場合は Node.js 24(OL8)/22(OL10) が固定されるため、複数バージョンのテストには `setup-node` アクションを使用するか、別のコンテナイメージを使用してください。
